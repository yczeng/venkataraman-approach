...
def evalUtteranceIterative(utterance):
	n = len(utterance)
	bestCost = [0] * n
	prevBoundary = [0] * n
	...
	for i in range(n):
		# need to add prev word to eval word
		bestCost[i] = evalWord(u[0:i])
		prevBoundary[i] = -1

		for j in range(i):
			prevWord = u[prevBoundary[j] + 1:i]
			word = u[j+1:i]
			cost = bestCost[j] + evalWord(prevWord, word)
...
# 		word = utterance[i::]
# 		print("SPLIT BY UTTERANCE AND WORD", subUtterance, word)
